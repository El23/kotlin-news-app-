...
object ISO8601Parser{
    fun parse(params:String): Date {
        ...
        if (input.endsWith("Z"))
            ...
            val inset = 6
            val start = input.subSequence(0, input.length - inset)
            val end = input.substring(input.length - input.length)

            input= StringBuilder(start).append("GMT").append(end).toString()
}
